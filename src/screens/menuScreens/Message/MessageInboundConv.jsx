...
const MessageInboundConv = () => {
  ...
  const { selectedConversation, conversations, loading, error } = useSelector(
  ...
  const defaultPlatforms = React.useMemo(
    () =>
      Array.isArray(connectors)
        ? connectors.map((connector) => connector.type.toLowerCase())
        : [],
    [connectors]
  );
      ...
      return (
        <div className="flex flex-col items-center justify-center h-full text-gray-500 p-4">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Select different channels or try again
          </p>
          <button
            onClick={handleRetry}
    ...
    return processedConversations.map((msg) => (
      <div
        ...
        className={`p-3 px-4 flex flex-col gap-1 cursor-pointer border-b border-gray hover:bg-gray-50 transition-colors duration-200
          ${
            selectedConv?.id === msg.id
              ? "border-l-4 border-gray bg-lightGray"
              : ""
          }`}
      ...
      >
        ...
        <div className="flex items-center justify-between gap-2">
          <p className="text-sm truncate flex-1 text-gray900">
            {msg.message?.substring(0, 100) || "No content"}
          </p>
        </div>
      </div>
    ));
